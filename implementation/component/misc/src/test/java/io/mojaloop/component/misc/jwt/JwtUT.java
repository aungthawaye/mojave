...
public class JwtUT {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(JwtUT.class);

    @Test
    public void testSign() throws NoSuchAlgorithmException, InvalidKeySpecException {

        var keyPair = KeyPairs.Rsa.generate(KeyPairs.Rsa.Size.SIZE_2048);

        var privateKey = KeyPairs.Rsa.privateKeyOf("-----BEGIN PRIVATE KEY-----\n" +
        ...
        var publicKey = KeyPairs.Rsa.publicKeyOf("-----BEGIN PUBLIC KEY-----\n" +
        ...
        for (int i = 0; i < 10; i++) {
            ...
            token = Jwt.sign(keyPair.getPrivate(), headers, payload);
        ...
        var ok = Jwt.verify(
            keyPair.getPublic(),
            new Jwt.Token(token.header(), encodedPayload, token.signature()));
        LOGGER.debug("ok : {}", ok);
