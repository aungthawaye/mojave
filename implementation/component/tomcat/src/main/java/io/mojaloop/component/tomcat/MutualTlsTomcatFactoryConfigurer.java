...
public class MutualTlsTomcatFactoryConfigurer {
    ...
    public static WebServerFactoryCustomizer<TomcatServletWebServerFactory> configure(ServerSettings settings,
        ...
        return factory -> {
            ...
            // ---------- SSL bundle (programmatic) ----------
            SslStoreBundle stores = SslStoreBundle.of(keyStore, keyStorePassword, trustStore);
            SslBundle sslBundle = SslBundle.of(stores);

            // Register this bundle under the name "tomcat"
            DefaultSslBundleRegistry bundleRegistry = new DefaultSslBundleRegistry(
                "tomcat", sslBundle);

            // Tell the server to use that bundle and require client certs
            Ssl ssl = Ssl.forBundle("tomcat");     // sets bundle name
            ssl.setClientAuth(Ssl.ClientAuth.NEED); // mTLS: client cert REQUIRED
            // (optional but explicit)
            ssl.setEnabled(true);

            factory.setSsl(ssl);
            factory.setSslBundles(bundleRegistry);
