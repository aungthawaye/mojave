...
public class RollbackReservationCommandIT extends BaseDomainIT {
    ...
    void should_fail_rollback_when_not_a_reservation() {
        ...
        final var input = new RollbackReservationCommand.Input(reservationId, rollbackUpdateId, "Rollback invalid");

        // Act & Assert
        assertThrows(RuntimeException.class, () -> this.rollbackReservationCommand.execute(input));
    }
    ...
    void should_rollback_reservation_successfully() {
        ...
        final var createOut = this.createPositionCommand.execute(
            new CreatePositionCommand.Input(new WalletOwnerId(89001L), Currency.USD, "Position RB", new BigDecimal("100.0000")));
        ...
        final var input = new RollbackReservationCommand.Input(reservationId, rollbackUpdateId, "Rollback reservation");
