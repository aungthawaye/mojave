...
@Component
public class DecreasePosition {

    private static final Logger LOGGER = LoggerFactory.getLogger(DecreasePosition.class);

    private final WalletIntercomService walletIntercomService;

    private final ObjectMapper objectMapper;

    public DecreasePosition(final WalletIntercomService walletIntercomService, final ObjectMapper objectMapper) {
    ...
    public DecreasePositionCommand.Output execute(final DecreasePositionCommand.Input input) throws WalletIntercomClientException {

        try {

            return RetrofitService.invoke(this.walletIntercomService.decreasePosition(input),
                                          (status, errorResponseBody) -> RestErrorResponse.decode(errorResponseBody, this.objectMapper)).body();

        } catch (RetrofitService.InvocationException e) {

            LOGGER.error("Error invoking decreasePosition : {}", e.getMessage());
