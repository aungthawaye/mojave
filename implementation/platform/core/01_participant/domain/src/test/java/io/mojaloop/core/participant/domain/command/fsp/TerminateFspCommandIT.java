...
import io.mojaloop.core.participant.contract.command.fsp.TerminateFspCommand;
...
class TerminateFspCommandIT extends BaseDomainIT {
    ...
    @Autowired
    private TerminateFspCommand terminateFsp;

    @Test
    void should_terminate_fsp() throws Exception {
        ...
        var createFspInput = new CreateFspCommand.Input(new FspCode("DFSP400"), "FSP-400", new Currency[]{Currency.USD}, endpoints);
        var created = this.createFsp.execute(createFspInput);

        var input = new TerminateFspCommand.Input(created.fspId());

        // Act
        var output = this.terminateFsp.execute(input);
    ...
    void should_throw_when_fsp_not_found() {
        ...
        // Arrange
        var input = new TerminateFspCommand.Input(new FspId(666_666L));

        // Act & Assert
        Assertions.assertThrows(FspIdNotFoundException.class, () -> this.terminateFsp.execute(input));
    }
