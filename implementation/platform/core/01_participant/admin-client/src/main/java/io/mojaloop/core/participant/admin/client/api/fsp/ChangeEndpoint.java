...
public class ChangeEndpoint {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(ChangeEndpoint.class);

    private final ParticipantAdminService.FspCommands fspCommands;

    private final ObjectMapper objectMapper;

    public ChangeEndpoint(ParticipantAdminService.FspCommands fspCommands, ObjectMapper objectMapper) {
    ...
    public ChangeEndpointCommand.Output execute(ChangeEndpointCommand.Input input) throws ParticipantAdminClientException {

        try {

            return RetrofitService.invoke(this.fspCommands.changeEndpoint(input), (status, errorResponseBody) -> RestErrorResponse.decode(errorResponseBody, this.objectMapper))
                                  .body();

        } catch (RetrofitService.InvocationException e) {

            LOGGER.error("Error invoking changeEndpoint : {}", e.getMessage());
