...
public class DeactivateEndpoint {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(DeactivateEndpoint.class);

    private final ParticipantAdminService.FspCommands fspCommands;

    private final ObjectMapper objectMapper;

    public DeactivateEndpoint(ParticipantAdminService.FspCommands fspCommands, ObjectMapper objectMapper) {
    ...
    public DeactivateEndpointCommand.Output execute(DeactivateEndpointCommand.Input input) throws ParticipantAdminClientException {

        try {

            return RetrofitService.invoke(this.fspCommands.deactivateEndpoint(input), (status, errorResponseBody) -> RestErrorResponse.decode(errorResponseBody, this.objectMapper))
                                  .body();

        } catch (RetrofitService.InvocationException e) {

            LOGGER.error("Error invoking deactivateEndpoint : {}", e.getMessage());
