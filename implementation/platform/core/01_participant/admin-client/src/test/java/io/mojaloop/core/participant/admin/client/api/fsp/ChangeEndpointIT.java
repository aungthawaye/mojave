...
public class ChangeEndpointIT {

    @Autowired
    private CreateFsp createFsp;

    @Autowired
    private ChangeEndpoint changeEndpoint;

    @Test
    public void test_successfully_change_endpoint() throws ParticipantAdminClientException {

        var output = this.createFsp.execute(new CreateFspCommand.Input(new FspCode("fsp-change-endpoint"), "FSP Change Endpoint", new Currency[]{Currency.USD},
