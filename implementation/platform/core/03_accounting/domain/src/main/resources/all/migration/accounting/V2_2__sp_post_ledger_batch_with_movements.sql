...
v_txn_at BIGINT;
    DECLARE
v_txn_type VARCHAR(32);
    DECLARE
v_flow_definition_id BIGINT;
    DECLARE
v_posting_definition_id BIGINT;
    DECLARE
done INT DEFAULT 0;

    -- Locked balance snapshot
    DECLARE
v_dr_curr DECIMAL(34, 4);
    DECLARE
v_cr_curr DECIMAL(34, 4);
    DECLARE
v_nature VARCHAR(16);
    DECLARE
v_mode VARCHAR(16);
...
v_limit DECIMAL(34, 4);
    DECLARE
v_signed_after DECIMAL(34, 4);

    -- Computed/apply
    DECLARE
v_dr_new DECIMAL(34, 4);
    DECLARE
v_cr_new DECIMAL(34, 4);

    -- Error code
    DECLARE
v_error INT DEFAULT 0; -- 0 = no error
    DECLARE
v_error_code VARCHAR(32);
    DECLARE
v_err_account_id BIGINT;
    DECLARE
v_err_side VARCHAR(32);
    DECLARE
v_err_currency VARCHAR(3);
    DECLARE
v_err_amount DECIMAL(34, 4);
    DECLARE
v_err_debits DECIMAL(34, 4);
    DECLARE
v_err_credits DECIMAL(34, 4);
...
CONTINUE HANDLER FOR NOT FOUND SET done = 1;
    ...
    /* ---------------- Early no-op ---------------- */
    IF
p_lines_json IS NULL OR JSON_TYPE(p_lines_json) <> 'ARRAY' OR
...
FROM acc_ledger_movement m
         JOIN tmp_lines t
              ON m.account_id = t.account_id
                  AND m.side = t.side
                  AND m.transaction_id =
                      t.transaction_id LIMIT 1; -- If multiple duplicates, just take the first one

SET
v_error = 1;
        SET
v_error_code = 'DUPLICATE_POSTING';
...
SET
v_error_code = COALESCE(v_error_code, 'SQL_ERROR');
...
END IF;
            ...
            IF
v_mode = 'FORBID' AND v_signed_after < 0 THEN
                ROLLBACK;
                SET
v_error = 1;
                SET
v_error_code = 'INSUFFICIENT_BALANCE';
                SET
v_err_account_id = v_account_id;
                SET
v_err_side = v_side;
                SET
v_err_currency = v_currency;
                SET
v_err_amount = v_amount;
                SET
v_err_debits = v_dr_curr;
                SET
v_err_credits = v_cr_curr;
                LEAVE
post_loop;
END IF;

            IF
v_mode = 'LIMITED' AND v_signed_after < -v_limit THEN
                ROLLBACK;
                SET
v_error = 1;
                SET
v_error_code = 'OVERDRAFT_EXCEEDED';
                SET
v_err_account_id = v_account_id;
                SET
v_err_side = v_side;
...
BEGIN
            DECLARE
r_done INT DEFAULT 0;
            DECLARE
r_ledger_movement_id BIGINT;
            DECLARE
r_step INT;
            DECLARE
r_account_id BIGINT;
...
r_currency VARCHAR(3);
            DECLARE
r_amount DECIMAL(34, 4);
            DECLARE
r_debits DECIMAL(34, 4);
            DECLARE
r_credits DECIMAL(34, 4);
...
ORDER BY step;
DECLARE
CONTINUE HANDLER FOR NOT FOUND SET r_done = 1;
