...
@Component
public class TerminateFlowDefinition {

    private static final Logger LOGGER = LoggerFactory.getLogger(TerminateFlowDefinition.class);

    private final AccountingAdminService.DefinitionCommands definitionCommands;

    private final ObjectMapper objectMapper;

    public TerminateFlowDefinition(AccountingAdminService.DefinitionCommands definitionCommands, ObjectMapper objectMapper) {
    ...
    public void execute(TerminateFlowDefinitionCommand.Input input) throws AccountingAdminClientException {

        try {

            RetrofitService
                .invoke(this.definitionCommands.terminateFlowDefinition(input),
                        (status, errorResponseBody) -> RestErrorResponse.decode(errorResponseBody, this.objectMapper));

        } catch (RetrofitService.InvocationException e) {

            LOGGER.error("Error invoking terminateFlowDefinition : {}", e.getMessage());
