...
@Component
public class RemoveFlowDefinitionPosting {

    private static final Logger LOGGER = LoggerFactory.getLogger(RemoveFlowDefinitionPosting.class);

    private final AccountingAdminService.DefinitionCommands definitionCommands;

    private final ObjectMapper objectMapper;

    public RemoveFlowDefinitionPosting(AccountingAdminService.DefinitionCommands definitionCommands, ObjectMapper objectMapper) {
    ...
    public void execute(RemoveFlowDefinitionPostingCommand.Input input) throws AccountingAdminClientException {

        try {

            RetrofitService
                .invoke(this.definitionCommands.removeFlowDefinitionPosting(input),
                        (status, errorResponseBody) -> RestErrorResponse.decode(errorResponseBody, this.objectMapper));

        } catch (RetrofitService.InvocationException e) {

            LOGGER.error("Error invoking removeFlowDefinitionPosting : {}", e.getMessage());
