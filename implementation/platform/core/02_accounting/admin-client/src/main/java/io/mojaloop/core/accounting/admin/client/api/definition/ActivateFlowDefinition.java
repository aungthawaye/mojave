...
@Component
public class ActivateFlowDefinition {

    private static final Logger LOGGER = LoggerFactory.getLogger(ActivateFlowDefinition.class);

    private final AccountingAdminService.DefinitionCommands definitionCommands;

    private final ObjectMapper objectMapper;

    public ActivateFlowDefinition(AccountingAdminService.DefinitionCommands definitionCommands, ObjectMapper objectMapper) {
    ...
    public void execute(ActivateFlowDefinitionCommand.Input input) throws AccountingAdminClientException {

        try {

            RetrofitService
                .invoke(this.definitionCommands.activateFlowDefinition(input),
                        (status, errorResponseBody) -> RestErrorResponse.decode(errorResponseBody, this.objectMapper));

        } catch (RetrofitService.InvocationException e) {

            LOGGER.error("Error invoking activateFlowDefinition : {}", e.getMessage());
