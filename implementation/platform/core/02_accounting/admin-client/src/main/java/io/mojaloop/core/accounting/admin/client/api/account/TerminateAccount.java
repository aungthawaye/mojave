...
public class TerminateAccount {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(TerminateAccount.class);

    private final AccountingAdminService.AccountCommands accountCommands;

    private final ObjectMapper objectMapper;

    public TerminateAccount(AccountingAdminService.AccountCommands accountCommands, ObjectMapper objectMapper) {
    ...
    public void execute(TerminateAccountCommand.Input input) throws AccountingAdminClientException {

        try {

            RetrofitService.invoke(this.accountCommands.terminateAccount(input), (status, errorResponseBody) -> RestErrorResponse.decode(errorResponseBody, this.objectMapper));

        } catch (RetrofitService.InvocationException e) {

            LOGGER.error("Error invoking terminateAccount : {}", e.getMessage());
