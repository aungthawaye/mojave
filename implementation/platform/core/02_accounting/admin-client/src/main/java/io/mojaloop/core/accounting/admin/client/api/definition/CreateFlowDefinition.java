...
@Component
public class CreateFlowDefinition {

    private static final Logger LOGGER = LoggerFactory.getLogger(CreateFlowDefinition.class);

    private final AccountingAdminService.DefinitionCommands definitionCommands;

    private final ObjectMapper objectMapper;

    public CreateFlowDefinition(AccountingAdminService.DefinitionCommands definitionCommands, ObjectMapper objectMapper) {
    ...
    public CreateFlowDefinitionCommand.Output execute(CreateFlowDefinitionCommand.Input input) throws AccountingAdminClientException {

        try {

            return RetrofitService.invoke(this.definitionCommands.createFlowDefinition(input),
                (status, errorResponseBody) -> RestErrorResponse.decode(errorResponseBody, this.objectMapper)).body();

        } catch (RetrofitService.InvocationException e) {

            LOGGER.error("Error invoking createFlowDefinition : {}", e.getMessage());
