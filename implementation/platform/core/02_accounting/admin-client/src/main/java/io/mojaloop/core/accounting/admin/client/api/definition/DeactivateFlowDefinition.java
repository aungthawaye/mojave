...
@Component
public class DeactivateFlowDefinition {

    private static final Logger LOGGER = LoggerFactory.getLogger(DeactivateFlowDefinition.class);

    private final AccountingAdminService.DefinitionCommands definitionCommands;

    private final ObjectMapper objectMapper;

    public DeactivateFlowDefinition(AccountingAdminService.DefinitionCommands definitionCommands, ObjectMapper objectMapper) {
    ...
    public void execute(DeactivateFlowDefinitionCommand.Input input) throws AccountingAdminClientException {

        try {

            RetrofitService
                .invoke(this.definitionCommands.deactivateFlowDefinition(input),
                        (status, errorResponseBody) -> RestErrorResponse.decode(errorResponseBody, this.objectMapper));

        } catch (RetrofitService.InvocationException e) {

            LOGGER.error("Error invoking deactivateFlowDefinition : {}", e.getMessage());
