...
public class AccountingAdminClientConfiguration {
    ...
    public AccountingAdminService.AccountCommands accountCommands(AccountingAdminService.Settings settings, ObjectMapper objectMapper) {
        ...
        return RetrofitService.newBuilder(AccountingAdminService.AccountCommands.class, settings.baseUrl()).withHttpLogging(
    ...
    public AccountingAdminService.ChartCommands chartCommands(AccountingAdminService.Settings settings, ObjectMapper objectMapper) {
        ...
        return RetrofitService.newBuilder(AccountingAdminService.ChartCommands.class, settings.baseUrl()).withHttpLogging(
            HttpLoggingInterceptor.Level.BODY, true).withDefaultFactories(objectMapper).build();
    }

    @Bean
    public AccountingAdminService.DefinitionCommands definitionCommands(AccountingAdminService.Settings settings,
                                                                        ObjectMapper objectMapper) {

        return RetrofitService.newBuilder(AccountingAdminService.DefinitionCommands.class, settings.baseUrl()).withHttpLogging(
            HttpLoggingInterceptor.Level.BODY, true).withDefaultFactories(objectMapper).build();
    }
