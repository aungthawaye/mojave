...
public class DeactivateAccountCommandIT extends BaseDomainIT {
    ...
    void should_deactivate_account_successfully() throws Exception {
        ...
        final var createInput = new CreateAccountCommand.Input(
            entryOut.chartEntryId(), new AccountOwnerId(2002L), Currency.USD, new AccountCode("ACC004"),
            "Account", "Desc", OverdraftMode.FORBID, BigDecimal.ZERO);
        final var accountOut = this.createAccountCommand.execute(createInput);

        final var input = new DeactivateAccountCommand.Input(accountOut.accountId());
    ...
    void should_fail_when_account_id_not_found() {
        // Arrange
        final var input = new DeactivateAccountCommand.Input(new AccountId(999999L));

        // Act & Assert
        assertThrows(AccountIdNotFoundException.class, () -> this.deactivateAccountCommand.execute(input));
    }
