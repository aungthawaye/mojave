...
public class RemovePostingDefinitionCommandIT extends BaseDomainIT {
    ...
    @Autowired
    private RemovePostingDefinitionCommand removePostingDefinitionCommand;
    ...
    void should_fail_when_flow_definition_not_found() {
        ...
        final var input = new RemovePostingDefinitionCommand.Input(invalidFlowId, invalidPostingId);

        // Act & Assert
        assertThrows(FlowDefinitionNotFoundException.class, () -> this.removePostingDefinitionCommand.execute(input));
    }
    ...
    void should_fail_when_posting_definition_not_found() throws Exception {
        ...
        this.createAccountCommand.execute(
            new CreateAccountCommand.Input(entry.chartEntryId(), new AccountOwnerId(3402L), Currency.USD, new AccountCode("ACC_ASSET"), "Asset Acc", "Test", OverdraftMode.FORBID,
                BigDecimal.ZERO));

        final var created = this.createFlowDefinitionCommand.execute(new CreateFlowDefinitionCommand.Input(TransactionType.FUND_IN, Currency.USD, "Flow G", "Desc", List.of(
        ...
        final var input = new RemovePostingDefinitionCommand.Input(created.flowDefinitionId(), new PostingDefinitionId(123456789L));

        // Act & Assert
        assertThrows(PostingDefinitionNotFoundException.class, () -> this.removePostingDefinitionCommand.execute(input));
    }
    ...
    void should_remove_posting_definition_successfully() throws Exception {
        ...
        // Mature entries
        this.createAccountCommand.execute(
            new CreateAccountCommand.Input(debitEntry.chartEntryId(), new AccountOwnerId(3401L), Currency.USD, new AccountCode("ACC_D"), "Debit Acc", "Test", OverdraftMode.FORBID,
                BigDecimal.ZERO));
        this.createAccountCommand.execute(
            new CreateAccountCommand.Input(creditEntry.chartEntryId(), new AccountOwnerId(3401L), Currency.USD, new AccountCode("ACC_C"), "Credit Acc", "Test",
        ...
        final var input = new RemovePostingDefinitionCommand.Input(created.flowDefinitionId(), toRemovePostingId);

        // Act
        final var output = this.removePostingDefinitionCommand.execute(input);
