...
public class AddPostingDefinitionCommandIT extends BaseDomainIT {
    ...
    void should_add_posting_definition_successfully() throws Exception {
        ...
        // Mature entries
        this.createAccountCommand.execute(new CreateAccountCommand.Input(debitEntry.chartEntryId(), new AccountOwnerId(3301L),
            Currency.USD, new AccountCode("ACC_D"), "Debit Acc", "Test", OverdraftMode.FORBID, BigDecimal.ZERO));
        this.createAccountCommand.execute(new CreateAccountCommand.Input(creditEntry.chartEntryId(), new AccountOwnerId(3301L),
        ...
        final var addPosting = new AddPostingDefinitionCommand.Input(
            ...
            new AddPostingDefinitionCommand.Input.Posting(ReceiveIn.CHART_ENTRY, creditEntry.chartEntryId().getId(), "DEPOSIT_INTO_FSP",
    ...
    void should_fail_when_chart_entry_is_immature() throws Exception {
        ...
        final var immatureEntry = this.createChartEntryCommand.execute(
            new CreateChartEntryCommand.Input(chartOut.chartId(), new ChartEntryCode("REV"), "Revenue", "Revenue Desc",
                AccountType.REVENUE));
        ...
        final var maturedEntry = this.createChartEntryCommand.execute(
            new CreateChartEntryCommand.Input(chartOut.chartId(), new ChartEntryCode("EXP"), "Expense", "Expense Desc",
                AccountType.EXPENSE));
        this.createAccountCommand.execute(new CreateAccountCommand.Input(maturedEntry.chartEntryId(), new AccountOwnerId(3302L),
            Currency.USD, new AccountCode("ACC_EXP"), "Expense Acc", "Test", OverdraftMode.FORBID, BigDecimal.ZERO));

        final var created = this.createFlowDefinitionCommand.execute(
            new CreateFlowDefinitionCommand.Input(TransactionType.FUND_OUT, Currency.USD, "Flow E", "Desc",
                List.of(new CreateFlowDefinitionCommand.Input.Posting(ReceiveIn.CHART_ENTRY, maturedEntry.chartEntryId().getId(), "WITHDRAW_FROM_FSP",
                    "LIQUIDITY_AMOUNT", Side.DEBIT, "Debit Expense"))));

        final var addPosting = new AddPostingDefinitionCommand.Input(
            created.flowDefinitionId(),
            new AddPostingDefinitionCommand.Input.Posting(ReceiveIn.CHART_ENTRY, immatureEntry.chartEntryId().getId(), "WITHDRAW_FROM_FSP",
                "LIQUIDITY_AMOUNT", Side.CREDIT, "Credit Revenue")
        );
