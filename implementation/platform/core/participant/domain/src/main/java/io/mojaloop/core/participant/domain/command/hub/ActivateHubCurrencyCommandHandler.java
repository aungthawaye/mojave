...
public class ActivateHubCurrencyCommandHandler implements ActivateHubCurrencyCommand {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(ActivateHubCurrencyCommandHandler.class);
    ...
    @Transactional
    @Write
    public Output execute(Input input) throws HubIdNotFoundException {

        LOGGER.info("Executing ActivateHubCurrencyCommand with input: {}", input);

        var hub = this.hubRepository
                      .findById(input.hubId())
                      .orElseThrow(() -> new HubIdNotFoundException(input.hubId()));

        boolean activated = hub.activateCurrency(input.currency());

        this.hubRepository.save(hub);

        LOGGER.info("Completed ActivateHubCurrencyCommand with input: {} -> activated={} ", input, activated);
