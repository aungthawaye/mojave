...
public class CreateOracleCommandIT {

    @Autowired
    private CreateOracleCommand createOracleCommand;

    @Test
    public void create_then_duplicateType_throws() throws Exception {

        assertNotNull(createOracleCommand);

        var out = createOracleCommand.execute(new CreateOracleCommand.Input(
            PartyIdType.MSISDN,
            "Oracle-MSISDN",
            "https://oracle-msisdn.example.com"
        ));
        assertNotNull(out);
        assertNotNull(out.oracleId());

        assertThrows(OracleAlreadyExistsException.class, () ->
                                                             createOracleCommand.execute(new CreateOracleCommand.Input(
                                                                 PartyIdType.MSISDN,
                                                                 "Another-MSISDN",
