...
public class ChangeOracleTypeIT {

    @Autowired
    private CreateOracle createOracle;

    @Autowired
    private ChangeOracleType changeOracleType;

    @Test
    public void test_successfully_change_oracle_type() throws ParticipantCommandClientException {
        var created = this.createOracle.execute(new CreateOracleCommand.Input(PartyIdType.PERSONAL_ID, "Oracle ChangeType", "http://localhost:7090"));
        this.changeOracleType.execute(new ChangeOracleTypeCommand.Input(new OracleId(created.oracleId().getId()), PartyIdType.MSISDN));
    }
}
