...
public class DeactivateFspIT {

    @Autowired
    private CreateFsp createFsp;

    @Autowired
    private DeactivateFsp deactivateFsp;

    @Test
    public void test_successfully_deactivate_fsp() throws ParticipantCommandClientException {
        var output = this.createFsp.execute(
            new CreateFspCommand.Input(new FspCode("fsp-deactivate"), "FSP Deactivate", new Currency[]{Currency.USD},
