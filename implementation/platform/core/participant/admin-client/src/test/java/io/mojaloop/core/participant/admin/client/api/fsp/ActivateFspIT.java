...
public class ActivateFspIT {

    @Autowired
    private CreateFsp createFsp;

    @Autowired
    private ActivateFsp activateFsp;

    @Test
    public void test_successfully_activate_fsp() throws ParticipantCommandClientException {
        var output = this.createFsp.execute(
            new CreateFspCommand.Input(new FspCode("fsp-activate"), "FSP Activate", new Currency[]{Currency.USD},
