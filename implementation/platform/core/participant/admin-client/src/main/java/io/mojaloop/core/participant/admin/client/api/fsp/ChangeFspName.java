...
@Component
public class ChangeFspName {

    private static final Logger LOGGER = LoggerFactory.getLogger(ChangeFspName.class);

    private final ParticipantAdminService.FspCommands fspCommands;

    private final ObjectMapper objectMapper;

    public ChangeFspName(ParticipantAdminService.FspCommands fspCommands, ObjectMapper objectMapper) {
    ...
    public ChangeFspNameCommand.Output execute(ChangeFspNameCommand.Input input) throws ParticipantCommandClientException {

        try {

            return RetrofitService
                       .invoke(this.fspCommands.changeFspName(input), (status, errorResponseBody) -> RestErrorResponse.decode(errorResponseBody, this.objectMapper))
                       .body();

        } catch (RetrofitService.InvocationException e) {

            LOGGER.error("Error invoking changeFspName : {}", e.getMessage());
