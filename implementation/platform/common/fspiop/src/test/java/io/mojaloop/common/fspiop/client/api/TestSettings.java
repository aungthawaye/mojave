package io.mojaloop.common.fspiop.client.api;

import io.mojaloop.common.fspiop.FspiopConfiguration;
import io.mojaloop.common.fspiop.client.FspiopClientConfiguration;
import io.mojaloop.common.fspiop.service.AccountLookUpService;
import io.mojaloop.common.fspiop.service.QuotingService;
import io.mojaloop.common.fspiop.service.TransferService;
import org.springframework.context.annotation.Bean;

import java.util.Map;

public class TestSettings implements FspiopClientConfiguration.RequiredSettings, FspiopConfiguration.RequiredSettings {

    @Bean
    @Override
    public AccountLookUpService.Settings accountLookUpServiceSettings() {

        return new AccountLookUpService.Settings("http://localhost:4002");
    }

    @Bean
    @Override
    public FspiopConfiguration.Settings fspiopConfigurationSettings() {

        return new FspiopConfiguration.Settings("fsp1",
                                                "FSP 1",
                                                "This_is_ILP_$ecret",
                                                true,
                                                true,
                                                "",
                                                Map.of("fsp2",
                                                       "",
                                                       "fsp3",
                                                       ""));
    }

    @Bean
    @Override
    public QuotingService.Settings quotingServiceSettings() {

        return new QuotingService.Settings("http://localhost:3002");
    }

    @Bean
    @Override
    public TransferService.Settings transferServiceSettings() {

        return new TransferService.Settings("http://localhost:3000");
    }

}
