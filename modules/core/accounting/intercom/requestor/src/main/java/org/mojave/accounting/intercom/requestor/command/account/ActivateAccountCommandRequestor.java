...
@Component
public class ActivateAccountCommandRequestor implements ActivateAccountCommand {

    private static final Logger LOGGER = LoggerFactory.getLogger(ActivateAccountCommandRequestor.class);

    private final Connection connection;

    private final ObjectMapper objectMapper;

    public ActivateAccountCommandRequestor(final Connection connection,
    ...
    public Output execute(final Input input) {
        ...
        LOGGER.info("ActivateAccountCommandRequestor : input: ({})", ObjectLogger.log(input));

        try {

            final var output = CommandRequestor.request(
                this.connection, ActivateAccountCommand.SUBJECT_NAME,
                input, Output.class, this.objectMapper);

            LOGGER.info("ActivateAccountCommandRequestor : output : ({})", ObjectLogger.log(output));
