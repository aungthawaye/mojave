...
@Component
public class ChangeCoaNameCommandRequestor implements ChangeCoaNameCommand {

    private static final Logger LOGGER = LoggerFactory.getLogger(ChangeCoaNameCommandRequestor.class);

    private final Connection connection;

    private final ObjectMapper objectMapper;

    public ChangeCoaNameCommandRequestor(final Connection connection,
    ...
    public Output execute(final Input input) {
        ...
        LOGGER.info("ChangeCoaNameCommandRequestor : input: ({})", ObjectLogger.log(input));

        try {

            final var output = NatsRequestor.request(
                this.connection, ChangeCoaNameCommand.SUBJECT_NAME,
                input, Output.class, this.objectMapper);

            LOGGER.info("ChangeCoaNameCommandRequestor : output : ({})", ObjectLogger.log(output));
