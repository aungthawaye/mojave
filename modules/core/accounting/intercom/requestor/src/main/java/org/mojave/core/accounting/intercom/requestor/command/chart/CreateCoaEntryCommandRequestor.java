...
@Component
public class CreateCoaEntryCommandRequestor implements CreateCoaEntryCommand {

    private static final Logger LOGGER = LoggerFactory.getLogger(CreateCoaEntryCommandRequestor.class);

    private final Connection connection;

    private final ObjectMapper objectMapper;

    public CreateCoaEntryCommandRequestor(final Connection connection,
    ...
    public Output execute(final Input input) {
        ...
        LOGGER.info("CreateCoaEntryCommandRequestor : input: ({})", ObjectLogger.log(input));

        try {

            final var output = NatsRequestor.request(
                this.connection, CreateCoaEntryCommand.SUBJECT_NAME,
                input, Output.class, this.objectMapper);

            LOGGER.info("CreateCoaEntryCommandRequestor : output : ({})", ObjectLogger.log(output));
