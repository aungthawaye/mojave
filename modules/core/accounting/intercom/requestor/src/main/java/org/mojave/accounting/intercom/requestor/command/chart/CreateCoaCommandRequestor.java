...
@Component
public class CreateCoaCommandRequestor implements CreateCoaCommand {

    private static final Logger LOGGER = LoggerFactory.getLogger(CreateCoaCommandRequestor.class);

    private final Connection connection;

    private final ObjectMapper objectMapper;

    public CreateCoaCommandRequestor(final Connection connection, final ObjectMapper objectMapper) {
    ...
    public Output execute(final Input input) {
        ...
        LOGGER.info("CreateCoaCommandRequestor : input: ({})", ObjectLogger.log(input));

        try {

            final var output = CommandRequestor.request(
                this.connection, CreateCoaCommand.SUBJECT_NAME,
                input, Output.class, this.objectMapper);

            LOGGER.info("CreateCoaCommandRequestor : output : ({})", ObjectLogger.log(output));
