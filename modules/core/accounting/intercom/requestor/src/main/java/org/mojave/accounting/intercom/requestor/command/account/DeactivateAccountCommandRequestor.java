...
@Component
public class DeactivateAccountCommandRequestor implements DeactivateAccountCommand {

    private static final Logger LOGGER = LoggerFactory.getLogger(DeactivateAccountCommandRequestor.class);

    private final Connection connection;

    private final ObjectMapper objectMapper;

    public DeactivateAccountCommandRequestor(final Connection connection,
    ...
    public Output execute(final Input input) {
        ...
        LOGGER.info("DeactivateAccountCommandRequestor : input: ({})", ObjectLogger.log(input));

        try {

            final var output = CommandRequestor.request(
                this.connection, DeactivateAccountCommand.SUBJECT_NAME,
                input, Output.class, this.objectMapper);

            LOGGER.info("DeactivateAccountCommandRequestor : output : ({})", ObjectLogger.log(output));
