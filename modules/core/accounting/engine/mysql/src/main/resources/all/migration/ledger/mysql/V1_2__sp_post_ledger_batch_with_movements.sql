...
CREATE PROCEDURE sp_post_ledger_batch_with_movements(IN p_lines_json JSON)
proc_posting:
BEGIN
    /* ---------------- Vars ---------------- */
    ...
    DECLARE c_lines CURSOR FOR SELECT ledger_movement_id,
                                      step,
                                      account_id,
                                      side,
                                      currency,
    ...
    FROM JSON_TABLE(p_lines_json, '$[*]'
                    COLUMNS (
                        ...
                        transaction_id BIGINT PATH '$.transactionId',
    ...
    IF v_error <> 0 THEN
        ...
        BEGIN
            ...
            DECLARE cur_rev CURSOR FOR SELECT ledger_movement_id,
                                              step,
                                              account_id,
                                              side,
                                              currency,
            ...
            LOOP
                ...
                BEGIN
                    ...
                    -- Update the ledgerOperation movement with the result REVERSED
                    UPDATE lgr_ledger_movement
