...
public class BaseIT {
    ...
    protected AccountId createAccount(final CreateAccountCommand createAccountCommand,
        ...
        final var output = createAccountCommand.execute(new CreateAccountCommand.Input(
            coaEntryId,
            new AccountOwnerId(ownerId),
            currency,
    ...
    protected CoaEntryId createCoaEntry(final CreateCoaEntryCommand createCoaEntryCommand,
                                        final CoaId coaId,
                                        final ChartEntryCategory category,
                                        final String code,
                                        final String name,
                                        final AccountType accountType) {

        final var output = createCoaEntryCommand.execute(new CreateCoaEntryCommand.Input(
            coaId,
            category,
            new CoaEntryCode(code),
            name,
            name + " description",
            accountType));
    ...
    protected FlowDefinitionId createFlowDefinition(
        ...
        final var output = createFlowDefinitionCommand.execute(new CreateFlowDefinitionCommand.Input(
            transactionType,
            currency,
            name,
            name + " description",
    ...
    protected void updateFlowDefinitionStatus(final FlowDefinitionId flowDefinitionId,
        ...
        try (final var connection = DriverManager.getConnection(
            WRITE_DB_URL, WRITE_DB_USER, WRITE_DB_PASSWORD);
