...
@Service
@Primary
public class AddFspGroupItemCommandHandler implements AddFspGroupItemCommand {

    private static final Logger LOGGER = LoggerFactory.getLogger(
        AddFspGroupItemCommandHandler.class);

    private final FspGroupRepository fspGroupRepository;

    private final FspRepository fspRepository;

    public AddFspGroupItemCommandHandler(final FspGroupRepository fspGroupRepository,
    ...
    public Output execute(final Input input) {
        ...
        LOGGER.info("AddFspGroupItemCommand : input: ({})", ObjectLogger.log(input));

        final var withId = FspRepository.Filters.withId(input.fspId());
        ...
        if (this.fspRepository.findOne(withId.and(alive)).isEmpty()) {
            throw new FspIdNotFoundException(input.fspId());
        }
        ...
        final var item = group.addItem(input.fspId());

        this.fspGroupRepository.save(group);

        final var output = new Output(item.getId());

        LOGGER.info("AddFspGroupItemCommand : output : ({})", ObjectLogger.log(output));
