...
@Component
public class JoinFspGroupCommandRequestor implements JoinFspGroupCommand {

    private static final Logger LOGGER = LoggerFactory.getLogger(JoinFspGroupCommandRequestor.class);

    private final Connection connection;

    private final ObjectMapper objectMapper;

    public JoinFspGroupCommandRequestor(final Connection connection, final ObjectMapper objectMapper) {
    ...
    public Output execute(final Input input) {
        ...
        LOGGER.info("JoinFspGroupCommandRequestor : input: ({})", ObjectLogger.log(input));

        try {

            final var output = CommandRequestor.request(
                this.connection, JoinFspGroupCommand.SUBJECT_NAME,
                input, Output.class, this.objectMapper);

            LOGGER.info("JoinFspGroupCommandRequestor : output : ({})", ObjectLogger.log(output));
