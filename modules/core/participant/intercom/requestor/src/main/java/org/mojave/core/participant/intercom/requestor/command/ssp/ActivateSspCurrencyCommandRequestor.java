...
@Component
public class ActivateSspCurrencyCommandRequestor implements ActivateSspCurrencyCommand {

    private static final Logger LOGGER = LoggerFactory.getLogger(ActivateSspCurrencyCommandRequestor.class);

    private final Connection connection;

    private final ObjectMapper objectMapper;

    public ActivateSspCurrencyCommandRequestor(final Connection connection, final ObjectMapper objectMapper) {
    ...
    public Output execute(final Input input) {
        ...
        LOGGER.info("ActivateSspCurrencyCommandRequestor : input: ({})", ObjectLogger.log(input));

        try {

            final var output = CommandRequestor.request(
                this.connection, ActivateSspCurrencyCommand.SUBJECT_NAME,
                input, Output.class, this.objectMapper);

            LOGGER.info("ActivateSspCurrencyCommandRequestor : output : ({})", ObjectLogger.log(output));
