...
@Component
public class ActivateHubCurrencyCommandRequestor implements ActivateHubCurrencyCommand {

    private static final Logger LOGGER = LoggerFactory.getLogger(ActivateHubCurrencyCommandRequestor.class);

    private final Connection connection;

    private final ObjectMapper objectMapper;

    public ActivateHubCurrencyCommandRequestor(final Connection connection, final ObjectMapper objectMapper) {
    ...
    public Output execute(final Input input) {
        ...
        LOGGER.info("ActivateHubCurrencyCommandRequestor : input: ({})", ObjectLogger.log(input));

        try {

            final var output = NatsRequestor.request(
                this.connection, ActivateHubCurrencyCommand.SUBJECT_NAME,
                input, Output.class, this.objectMapper);

            LOGGER.info("ActivateHubCurrencyCommandRequestor : output : ({})", ObjectLogger.log(output));
