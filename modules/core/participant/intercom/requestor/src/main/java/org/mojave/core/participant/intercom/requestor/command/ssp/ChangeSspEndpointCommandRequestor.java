...
@Component
public class ChangeSspEndpointCommandRequestor implements ChangeSspEndpointCommand {

    private static final Logger LOGGER = LoggerFactory.getLogger(ChangeSspEndpointCommandRequestor.class);

    private final Connection connection;

    private final ObjectMapper objectMapper;

    public ChangeSspEndpointCommandRequestor(final Connection connection, final ObjectMapper objectMapper) {
    ...
    public Output execute(final Input input) {
        ...
        LOGGER.info("ChangeSspEndpointCommandRequestor : input: ({})", ObjectLogger.log(input));

        try {

            final var output = NatsRequestor.request(
                this.connection, ChangeSspEndpointCommand.SUBJECT_NAME,
                input, Output.class, this.objectMapper);

            LOGGER.info("ChangeSspEndpointCommandRequestor : output : ({})", ObjectLogger.log(output));
