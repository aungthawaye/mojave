...
@Component
public class ChangeOracleNameCommandRequestor implements ChangeOracleNameCommand {

    private static final Logger LOGGER = LoggerFactory.getLogger(ChangeOracleNameCommandRequestor.class);

    private final Connection connection;

    private final ObjectMapper objectMapper;

    public ChangeOracleNameCommandRequestor(final Connection connection, final ObjectMapper objectMapper) {
    ...
    public Output execute(final Input input) {
        ...
        LOGGER.info("ChangeOracleNameCommandRequestor : input: ({})", ObjectLogger.log(input));

        try {

            final var output = NatsRequestor.request(
                this.connection, ChangeOracleNameCommand.SUBJECT_NAME,
                input, Output.class, this.objectMapper);

            LOGGER.info("ChangeOracleNameCommandRequestor : output : ({})", ObjectLogger.log(output));
