...
@Component
public class LeaveFspGroupCommandRequestor implements LeaveFspGroupCommand {

    private static final Logger LOGGER = LoggerFactory.getLogger(LeaveFspGroupCommandRequestor.class);

    private final Connection connection;

    private final ObjectMapper objectMapper;

    public LeaveFspGroupCommandRequestor(final Connection connection, final ObjectMapper objectMapper) {
    ...
    public Output execute(final Input input) {
        ...
        LOGGER.info("LeaveFspGroupCommandRequestor : input: ({})", ObjectLogger.log(input));

        try {

            final var output = CommandRequestor.request(
                this.connection, LeaveFspGroupCommand.SUBJECT_NAME,
                input, Output.class, this.objectMapper);

            LOGGER.info("LeaveFspGroupCommandRequestor : output : ({})", ObjectLogger.log(output));
