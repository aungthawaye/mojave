...
@Component
public class ActivateSspCommandRequestor implements ActivateSspCommand {

    private static final Logger LOGGER = LoggerFactory.getLogger(ActivateSspCommandRequestor.class);

    private final Connection connection;

    private final ObjectMapper objectMapper;

    public ActivateSspCommandRequestor(final Connection connection, final ObjectMapper objectMapper) {
    ...
    public Output execute(final Input input) {
        ...
        LOGGER.info("ActivateSspCommandRequestor : input: ({})", ObjectLogger.log(input));

        try {

            final var output = CommandRequestor.request(
                this.connection, ActivateSspCommand.SUBJECT_NAME,
                input, Output.class, this.objectMapper);

            LOGGER.info("ActivateSspCommandRequestor : output : ({})", ObjectLogger.log(output));
