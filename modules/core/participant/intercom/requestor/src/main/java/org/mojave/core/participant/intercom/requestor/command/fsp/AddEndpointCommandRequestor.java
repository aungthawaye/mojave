...
@Component
public class AddEndpointCommandRequestor implements AddEndpointCommand {

    private static final Logger LOGGER = LoggerFactory.getLogger(AddEndpointCommandRequestor.class);

    private final Connection connection;

    private final ObjectMapper objectMapper;

    public AddEndpointCommandRequestor(final Connection connection, final ObjectMapper objectMapper) {
    ...
    public Output execute(final Input input) {
        ...
        LOGGER.info("AddEndpointCommandRequestor : input: ({})", ObjectLogger.log(input));

        try {

            final var output = CommandRequestor.request(
                this.connection, AddEndpointCommand.SUBJECT_NAME,
                input, Output.class, this.objectMapper);

            LOGGER.info("AddEndpointCommandRequestor : output : ({})", ObjectLogger.log(output));
