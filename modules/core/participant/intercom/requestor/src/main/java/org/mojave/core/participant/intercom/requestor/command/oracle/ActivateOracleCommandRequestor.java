...
@Component
public class ActivateOracleCommandRequestor implements ActivateOracleCommand {

    private static final Logger LOGGER = LoggerFactory.getLogger(ActivateOracleCommandRequestor.class);

    private final Connection connection;

    private final ObjectMapper objectMapper;

    public ActivateOracleCommandRequestor(final Connection connection, final ObjectMapper objectMapper) {
    ...
    public Output execute(final Input input) {
        ...
        LOGGER.info("ActivateOracleCommandRequestor : input: ({})", ObjectLogger.log(input));

        try {

            final var output = NatsRequestor.request(
                this.connection, ActivateOracleCommand.SUBJECT_NAME,
                input, Output.class, this.objectMapper);

            LOGGER.info("ActivateOracleCommandRequestor : output : ({})", ObjectLogger.log(output));
