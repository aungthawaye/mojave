...
@Component
public class TerminateFspCommandRequestor implements TerminateFspCommand {

    private static final Logger LOGGER = LoggerFactory.getLogger(TerminateFspCommandRequestor.class);

    private final Connection connection;

    private final ObjectMapper objectMapper;

    public TerminateFspCommandRequestor(final Connection connection, final ObjectMapper objectMapper) {
    ...
    public Output execute(final Input input) {
        ...
        LOGGER.info("TerminateFspCommandRequestor : input: ({})", ObjectLogger.log(input));

        try {

            final var output = CommandRequestor.request(
                this.connection, TerminateFspCommand.SUBJECT_NAME,
                input, Output.class, this.objectMapper);

            LOGGER.info("TerminateFspCommandRequestor : output : ({})", ObjectLogger.log(output));
