...
@Component
public class ChangeHubNameCommandRequestor implements ChangeHubNameCommand {

    private static final Logger LOGGER = LoggerFactory.getLogger(ChangeHubNameCommandRequestor.class);

    private final Connection connection;

    private final ObjectMapper objectMapper;

    public ChangeHubNameCommandRequestor(final Connection connection, final ObjectMapper objectMapper) {
    ...
    public Output execute(final Input input) {
        ...
        LOGGER.info("ChangeHubNameCommandRequestor : input: ({})", ObjectLogger.log(input));

        try {

            final var output = NatsRequestor.request(
                this.connection, ChangeHubNameCommand.SUBJECT_NAME,
                input, Output.class, this.objectMapper);

            LOGGER.info("ChangeHubNameCommandRequestor : output : ({})", ObjectLogger.log(output));
