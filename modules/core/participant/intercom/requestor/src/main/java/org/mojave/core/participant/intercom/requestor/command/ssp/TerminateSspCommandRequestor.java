...
@Component
public class TerminateSspCommandRequestor implements TerminateSspCommand {

    private static final Logger LOGGER = LoggerFactory.getLogger(TerminateSspCommandRequestor.class);

    private final Connection connection;

    private final ObjectMapper objectMapper;

    public TerminateSspCommandRequestor(final Connection connection, final ObjectMapper objectMapper) {
    ...
    public Output execute(final Input input) {
        ...
        LOGGER.info("TerminateSspCommandRequestor : input: ({})", ObjectLogger.log(input));

        try {

            final var output = NatsRequestor.request(
                this.connection, TerminateSspCommand.SUBJECT_NAME,
                input, Output.class, this.objectMapper);

            LOGGER.info("TerminateSspCommandRequestor : output : ({})", ObjectLogger.log(output));
