...
@Component
public class CreateHubCommandRequestor implements CreateHubCommand {

    private static final Logger LOGGER = LoggerFactory.getLogger(CreateHubCommandRequestor.class);

    private final Connection connection;

    private final ObjectMapper objectMapper;

    public CreateHubCommandRequestor(final Connection connection, final ObjectMapper objectMapper) {
    ...
    public Output execute(final Input input) {
        ...
        LOGGER.info("CreateHubCommandRequestor : input: ({})", ObjectLogger.log(input));

        try {

            final var output = NatsRequestor.request(
                this.connection, CreateHubCommand.SUBJECT_NAME,
                input, Output.class, this.objectMapper);

            LOGGER.info("CreateHubCommandRequestor : output : ({})", ObjectLogger.log(output));
