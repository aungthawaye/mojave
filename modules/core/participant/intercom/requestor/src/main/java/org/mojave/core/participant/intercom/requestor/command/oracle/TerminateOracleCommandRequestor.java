...
@Component
public class TerminateOracleCommandRequestor implements TerminateOracleCommand {

    private static final Logger LOGGER = LoggerFactory.getLogger(TerminateOracleCommandRequestor.class);

    private final Connection connection;

    private final ObjectMapper objectMapper;

    public TerminateOracleCommandRequestor(final Connection connection, final ObjectMapper objectMapper) {
    ...
    public Output execute(final Input input) {
        ...
        LOGGER.info("TerminateOracleCommandRequestor : input: ({})", ObjectLogger.log(input));

        try {

            final var output = NatsRequestor.request(
                this.connection, TerminateOracleCommand.SUBJECT_NAME,
                input, Output.class, this.objectMapper);

            LOGGER.info("TerminateOracleCommandRequestor : output : ({})", ObjectLogger.log(output));
