...
@Component
public class ActivateEndpointCommandRequestor implements ActivateEndpointCommand {

    private static final Logger LOGGER = LoggerFactory.getLogger(ActivateEndpointCommandRequestor.class);

    private final Connection connection;

    private final ObjectMapper objectMapper;

    public ActivateEndpointCommandRequestor(final Connection connection, final ObjectMapper objectMapper) {
    ...
    public Output execute(final Input input) {
        ...
        LOGGER.info("ActivateEndpointCommandRequestor : input: ({})", ObjectLogger.log(input));

        try {

            final var output = CommandRequestor.request(
                this.connection, ActivateEndpointCommand.SUBJECT_NAME,
                input, Output.class, this.objectMapper);

            LOGGER.info("ActivateEndpointCommandRequestor : output : ({})", ObjectLogger.log(output));
