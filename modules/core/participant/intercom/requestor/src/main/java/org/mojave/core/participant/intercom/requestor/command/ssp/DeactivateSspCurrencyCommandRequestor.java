...
@Component
public class DeactivateSspCurrencyCommandRequestor implements DeactivateSspCurrencyCommand {

    private static final Logger LOGGER = LoggerFactory.getLogger(DeactivateSspCurrencyCommandRequestor.class);

    private final Connection connection;

    private final ObjectMapper objectMapper;

    public DeactivateSspCurrencyCommandRequestor(final Connection connection, final ObjectMapper objectMapper) {
    ...
    public Output execute(final Input input) {
        ...
        LOGGER.info("DeactivateSspCurrencyCommandRequestor : input: ({})", ObjectLogger.log(input));

        try {

            final var output = NatsRequestor.request(
                this.connection, DeactivateSspCurrencyCommand.SUBJECT_NAME,
                input, Output.class, this.objectMapper);

            LOGGER.info("DeactivateSspCurrencyCommandRequestor : output : ({})", ObjectLogger.log(output));
