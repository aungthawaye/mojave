...
@Component
public class DeactivateFspCurrencyCommandRequestor implements DeactivateFspCurrencyCommand {

    private static final Logger LOGGER = LoggerFactory.getLogger(DeactivateFspCurrencyCommandRequestor.class);

    private final Connection connection;

    private final ObjectMapper objectMapper;

    public DeactivateFspCurrencyCommandRequestor(final Connection connection, final ObjectMapper objectMapper) {
    ...
    public Output execute(final Input input) {
        ...
        LOGGER.info("DeactivateFspCurrencyCommandRequestor : input: ({})", ObjectLogger.log(input));

        try {

            final var output = CommandRequestor.request(
                this.connection, DeactivateFspCurrencyCommand.SUBJECT_NAME,
                input, Output.class, this.objectMapper);

            LOGGER.info("DeactivateFspCurrencyCommandRequestor : output : ({})", ObjectLogger.log(output));
