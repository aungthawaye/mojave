...
@Component
public class ChangeFspNameCommandRequestor implements ChangeFspNameCommand {

    private static final Logger LOGGER = LoggerFactory.getLogger(ChangeFspNameCommandRequestor.class);

    private final Connection connection;

    private final ObjectMapper objectMapper;

    public ChangeFspNameCommandRequestor(final Connection connection, final ObjectMapper objectMapper) {
    ...
    public Output execute(final Input input) {
        ...
        LOGGER.info("ChangeFspNameCommandRequestor : input: ({})", ObjectLogger.log(input));

        try {

            final var output = NatsRequestor.request(
                this.connection, ChangeFspNameCommand.SUBJECT_NAME,
                input, Output.class, this.objectMapper);

            LOGGER.info("ChangeFspNameCommandRequestor : output : ({})", ObjectLogger.log(output));
