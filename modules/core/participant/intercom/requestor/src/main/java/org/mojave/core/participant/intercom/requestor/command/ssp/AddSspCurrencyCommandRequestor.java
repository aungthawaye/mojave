...
@Component
public class AddSspCurrencyCommandRequestor implements AddSspCurrencyCommand {

    private static final Logger LOGGER = LoggerFactory.getLogger(AddSspCurrencyCommandRequestor.class);

    private final Connection connection;

    private final ObjectMapper objectMapper;

    public AddSspCurrencyCommandRequestor(final Connection connection, final ObjectMapper objectMapper) {
    ...
    public Output execute(final Input input) {
        ...
        LOGGER.info("AddSspCurrencyCommandRequestor : input: ({})", ObjectLogger.log(input));

        try {

            final var output = NatsRequestor.request(
                this.connection, AddSspCurrencyCommand.SUBJECT_NAME,
                input, Output.class, this.objectMapper);

            LOGGER.info("AddSspCurrencyCommandRequestor : output : ({})", ObjectLogger.log(output));
