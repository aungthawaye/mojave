...
@Component
public class CreateSspCommandRequestor implements CreateSspCommand {

    private static final Logger LOGGER = LoggerFactory.getLogger(CreateSspCommandRequestor.class);

    private final Connection connection;

    private final ObjectMapper objectMapper;

    public CreateSspCommandRequestor(final Connection connection, final ObjectMapper objectMapper) {
    ...
    public Output execute(final Input input) {
        ...
        LOGGER.info("CreateSspCommandRequestor : input: ({})", ObjectLogger.log(input));

        try {

            final var output = NatsRequestor.request(
                this.connection, CreateSspCommand.SUBJECT_NAME,
                input, Output.class, this.objectMapper);

            LOGGER.info("CreateSspCommandRequestor : output : ({})", ObjectLogger.log(output));
