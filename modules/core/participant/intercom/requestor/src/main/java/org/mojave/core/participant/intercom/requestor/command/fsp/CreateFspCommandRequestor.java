...
@Component
public class CreateFspCommandRequestor implements CreateFspCommand {

    private static final Logger LOGGER = LoggerFactory.getLogger(CreateFspCommandRequestor.class);

    private final Connection connection;

    private final ObjectMapper objectMapper;

    public CreateFspCommandRequestor(final Connection connection, final ObjectMapper objectMapper) {
    ...
    public Output execute(final Input input) {
        ...
        LOGGER.info("CreateFspCommandRequestor : input: ({})", ObjectLogger.log(input));

        try {

            final var output = NatsRequestor.request(
                this.connection, CreateFspCommand.SUBJECT_NAME,
                input, Output.class, this.objectMapper);

            LOGGER.info("CreateFspCommandRequestor : output : ({})", ObjectLogger.log(output));
