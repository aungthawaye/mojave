...
@Component
public class CreateFspGroupCommandRequestor implements CreateFspGroupCommand {

    private static final Logger LOGGER = LoggerFactory.getLogger(CreateFspGroupCommandRequestor.class);

    private final Connection connection;

    private final ObjectMapper objectMapper;

    public CreateFspGroupCommandRequestor(final Connection connection, final ObjectMapper objectMapper) {
    ...
    public Output execute(final Input input) {
        ...
        LOGGER.info("CreateFspGroupCommandRequestor : input: ({})", ObjectLogger.log(input));

        try {

            final var output = NatsRequestor.request(
                this.connection, CreateFspGroupCommand.SUBJECT_NAME,
                input, Output.class, this.objectMapper);

            LOGGER.info("CreateFspGroupCommandRequestor : output : ({})", ObjectLogger.log(output));
