...
@Component
public class ActivateFspCurrencyCommandRequestor implements ActivateFspCurrencyCommand {

    private static final Logger LOGGER = LoggerFactory.getLogger(ActivateFspCurrencyCommandRequestor.class);

    private final Connection connection;

    private final ObjectMapper objectMapper;

    public ActivateFspCurrencyCommandRequestor(final Connection connection, final ObjectMapper objectMapper) {
    ...
    public Output execute(final Input input) {
        ...
        LOGGER.info("ActivateFspCurrencyCommandRequestor : input: ({})", ObjectLogger.log(input));

        try {

            final var output = NatsRequestor.request(
                this.connection, ActivateFspCurrencyCommand.SUBJECT_NAME,
                input, Output.class, this.objectMapper);

            LOGGER.info("ActivateFspCurrencyCommandRequestor : output : ({})", ObjectLogger.log(output));
