...
@Component
public class ChangeOracleTypeCommandRequestor implements ChangeOracleTypeCommand {

    private static final Logger LOGGER = LoggerFactory.getLogger(ChangeOracleTypeCommandRequestor.class);

    private final Connection connection;

    private final ObjectMapper objectMapper;

    public ChangeOracleTypeCommandRequestor(final Connection connection, final ObjectMapper objectMapper) {
    ...
    public Output execute(final Input input) {
        ...
        LOGGER.info("ChangeOracleTypeCommandRequestor : input: ({})", ObjectLogger.log(input));

        try {

            final var output = NatsRequestor.request(
                this.connection, ChangeOracleTypeCommand.SUBJECT_NAME,
                input, Output.class, this.objectMapper);

            LOGGER.info("ChangeOracleTypeCommandRequestor : output : ({})", ObjectLogger.log(output));
