package org.mojave.core.participant.intercom.replier.query.fsp;

import io.nats.client.Connection;
import io.nats.client.Message;
import org.mojave.component.misc.error.MojaveErrorResponse;
import org.mojave.component.misc.logger.ObjectLogger;
import org.mojave.component.nats.Envelope;
import org.mojave.core.participant.contract.query.FspGroupQuery;
import org.slf4j.Logger;
import org.slf4j.LoggerFactory;
import org.springframework.stereotype.Component;
import tools.jackson.databind.ObjectMapper;

import java.util.Objects;

@Component
public class FspGroupQueryReplier {

    private static final Logger LOGGER = LoggerFactory.getLogger(FspGroupQueryReplier.class);

    private final FspGroupQuery fspGroupQuery;

    private final Connection connection;

    private final ObjectMapper objectMapper;

    public FspGroupQueryReplier(final FspGroupQuery fspGroupQuery,
                                final Connection connection,
                                final ObjectMapper objectMapper) {

        Objects.requireNonNull(fspGroupQuery);
        Objects.requireNonNull(connection);
        Objects.requireNonNull(objectMapper);

        this.fspGroupQuery = fspGroupQuery;
        this.connection = connection;
        this.objectMapper = objectMapper;

        this.connection.createDispatcher(this::handleGetById).subscribe(FspGroupQuery.GET_BY_ID_SUBJECT_NAME);
        this.connection.createDispatcher(this::handleGetAll).subscribe(FspGroupQuery.GET_ALL_SUBJECT_NAME);
    }

    private void handleGetById(final Message message) {

        this.reply(message, FspGroupQuery.GetByIdInput.class, input -> {
            LOGGER.info("FspGroupQueryReplier.get : input: ({})", ObjectLogger.log(input));
            final var output = this.fspGroupQuery.get(input.fspGroupId());
            LOGGER.info("FspGroupQueryReplier.get : output : ({})", ObjectLogger.log(output));
            return output;
        });
    }

    private void handleGetAll(final Message message) {

        this.reply(message, FspGroupQuery.GetAllInput.class, input -> {
            LOGGER.info("FspGroupQueryReplier.getAll : input: ({})", ObjectLogger.log(input));
            final var output = this.fspGroupQuery.getAll();
            LOGGER.info("FspGroupQueryReplier.getAll : output : ({})", ObjectLogger.log(output));
            return output;
        });
    }

    private <I> void reply(final Message message,
                           final Class<I> inputType,
                           final Handler<I> handler) {

        final var replyTo = message.getReplyTo();

        if (replyTo == null || replyTo.isBlank()) {
            LOGGER.warn("FspGroupQueryReplier : reply subject is missing.");
            return;
        }

        try {

            final var input = this.objectMapper.readValue(message.getData(), inputType);
            final var output = handler.handle(input);
            final var response = Envelope.success(this.objectMapper.valueToTree(output));
            final var responseData = this.objectMapper.writeValueAsBytes(response);

            this.connection.publish(replyTo, responseData);

        } catch (final Exception exception) {

            final var output = MojaveErrorResponse.from(exception);
            final var response = Envelope.failure(this.objectMapper.valueToTree(output));
            final var responseData = this.objectMapper.writeValueAsBytes(response);

            this.connection.publish(replyTo, responseData);
        }
    }

    @FunctionalInterface
    private interface Handler<I> {

        Object handle(I input) throws Exception;

    }

}
