...
@Component
public class JoinFspGroupCommandReplier {

    private static final Logger LOGGER = LoggerFactory.getLogger(JoinFspGroupCommandReplier.class);
    ...
    public JoinFspGroupCommandReplier(final JoinFspGroupCommand joinFspGroupCommand,
        ...
        this.connection.createDispatcher(this::handle).subscribe(JoinFspGroupCommand.SUBJECT_NAME);
    }
    ...
    private void handle(final Message message) {
        ...
        if (replyTo == null || replyTo.isBlank()) {
            LOGGER.warn("JoinFspGroupCommandReplier : reply subject is missing.");
            return;
        }
        ...
        try {
            ...
            final var input = this.objectMapper.readValue(
                message.getData(), JoinFspGroupCommand.Input.class);

            LOGGER.info("JoinFspGroupCommandReplier : input: ({})", ObjectLogger.log(input));

            final var output = this.joinFspGroupCommand.execute(input);
            ...
            LOGGER.info("JoinFspGroupCommandReplier : output : ({})", ObjectLogger.log(output));
