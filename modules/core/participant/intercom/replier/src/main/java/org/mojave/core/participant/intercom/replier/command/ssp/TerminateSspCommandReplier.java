...
@Component
public class TerminateSspCommandReplier {

    private static final Logger LOGGER = LoggerFactory.getLogger(TerminateSspCommandReplier.class);
    ...
    public TerminateSspCommandReplier(final TerminateSspCommand terminateSspCommand,
        ...
        this.connection.createDispatcher(this::handle).subscribe(TerminateSspCommand.SUBJECT_NAME);
    }
    ...
    private void handle(final Message message) {
        ...
        if (replyTo == null || replyTo.isBlank()) {
            LOGGER.warn("TerminateSspCommandReplier : reply subject is missing.");
            return;
        }
        ...
        try {
            ...
            final var input = this.objectMapper.readValue(
                message.getData(), TerminateSspCommand.Input.class);

            LOGGER.info("TerminateSspCommandReplier : input: ({})", ObjectLogger.log(input));

            final var output = this.terminateSspCommand.execute(input);
            ...
            LOGGER.info("TerminateSspCommandReplier : output : ({})", ObjectLogger.log(output));
