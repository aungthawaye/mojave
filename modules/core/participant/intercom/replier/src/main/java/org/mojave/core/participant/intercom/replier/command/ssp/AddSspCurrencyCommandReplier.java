...
@Component
public class AddSspCurrencyCommandReplier {

    private static final Logger LOGGER = LoggerFactory.getLogger(AddSspCurrencyCommandReplier.class);
    ...
    public AddSspCurrencyCommandReplier(final AddSspCurrencyCommand addSspCurrencyCommand,
        ...
        this.connection.createDispatcher(this::handle).subscribe(AddSspCurrencyCommand.SUBJECT_NAME);
    }
    ...
    private void handle(final Message message) {
        ...
        if (replyTo == null || replyTo.isBlank()) {
            LOGGER.warn("AddSspCurrencyCommandReplier : reply subject is missing.");
            return;
        }
        ...
        try {
            ...
            final var input = this.objectMapper.readValue(
                message.getData(), AddSspCurrencyCommand.Input.class);

            LOGGER.info("AddSspCurrencyCommandReplier : input: ({})", ObjectLogger.log(input));

            final var output = this.addSspCurrencyCommand.execute(input);
            ...
            LOGGER.info("AddSspCurrencyCommandReplier : output : ({})", ObjectLogger.log(output));
