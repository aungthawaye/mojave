...
@Component
public class ChangeFspEndpointCommandReplier {

    private static final Logger LOGGER = LoggerFactory.getLogger(ChangeFspEndpointCommandReplier.class);
    ...
    public ChangeFspEndpointCommandReplier(final ChangeFspEndpointCommand changeFspEndpointCommand,
        ...
        this.connection.createDispatcher(this::handle).subscribe(ChangeFspEndpointCommand.SUBJECT_NAME);
    }
    ...
    private void handle(final Message message) {
        ...
        if (replyTo == null || replyTo.isBlank()) {
            LOGGER.warn("ChangeFspEndpointCommandReplier : reply subject is missing.");
            return;
        }
        ...
        try {
            ...
            final var input = this.objectMapper.readValue(
                message.getData(), ChangeFspEndpointCommand.Input.class);

            LOGGER.info("ChangeFspEndpointCommandReplier : input: ({})", ObjectLogger.log(input));

            final var output = this.changeFspEndpointCommand.execute(input);
            ...
            LOGGER.info("ChangeFspEndpointCommandReplier : output : ({})", ObjectLogger.log(output));
