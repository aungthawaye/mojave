...
@Component
public class FulfilPositionsCommandRequestor implements FulfilPositionsCommand {

    private static final Logger LOGGER = LoggerFactory.getLogger(FulfilPositionsCommandRequestor.class);

    private final Connection connection;

    private final ObjectMapper objectMapper;

    public FulfilPositionsCommandRequestor(final Connection connection,
    ...
    public Output execute(final Input input) throws FailedToFulfilPositionsException {
        ...
        LOGGER.info("FulfilPositionsCommandRequestor : input: ({})", ObjectLogger.log(input));

        try {

            final var output = CommandRequestor.request(
                this.connection, FulfilPositionsCommand.SUBJECT_NAME,
                input, Output.class, this.objectMapper);

            LOGGER.info("FulfilPositionsCommandRequestor : output : ({})", ObjectLogger.log(output));
            ...
            if (decodedErrorResponse instanceof final MojaveErrorResponse errorResponse) {
                ...
                if (throwable instanceof final FailedToFulfilPositionsException failedToFulfilPositionsException) {
                    throw failedToFulfilPositionsException;
                }
