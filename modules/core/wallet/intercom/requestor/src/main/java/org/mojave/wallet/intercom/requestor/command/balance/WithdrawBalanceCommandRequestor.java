...
@Component
public class WithdrawBalanceCommandRequestor implements WithdrawBalanceCommand {

    private static final Logger LOGGER = LoggerFactory.getLogger(WithdrawBalanceCommandRequestor.class);

    private final Connection connection;

    private final ObjectMapper objectMapper;

    public WithdrawBalanceCommandRequestor(final Connection connection,
    ...
    public Output execute(final Input input)
        ...
        LOGGER.info("WithdrawBalanceCommandRequestor : input: ({})", ObjectLogger.log(input));

        try {

            final var output = CommandRequestor.request(
                this.connection, WithdrawBalanceCommand.SUBJECT_NAME,
                input, Output.class, this.objectMapper);

            LOGGER.info("WithdrawBalanceCommandRequestor : output : ({})", ObjectLogger.log(output));
            ...
            if (decodedErrorResponse instanceof final MojaveErrorResponse errorResponse) {
                ...
                if (throwable instanceof final InsufficientBalanceException insufficientBalanceException) {
