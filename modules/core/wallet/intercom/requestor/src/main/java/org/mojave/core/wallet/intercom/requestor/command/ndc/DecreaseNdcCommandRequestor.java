...
@Component
public class DecreaseNdcCommandRequestor implements DecreaseNdcCommand {

    private static final Logger LOGGER = LoggerFactory.getLogger(DecreaseNdcCommandRequestor.class);

    private final Connection connection;

    private final ObjectMapper objectMapper;

    public DecreaseNdcCommandRequestor(final Connection connection,
    ...
    public Output execute(final Input input) throws
        ...
        LOGGER.info("DecreaseNdcCommandRequestor : input: ({})", ObjectLogger.log(input));

        try {

            final var output = NatsRequestor.request(
                this.connection, DecreaseNdcCommand.SUBJECT_NAME,
                input, Output.class, this.objectMapper);

            LOGGER.info("DecreaseNdcCommandRequestor : output : ({})", ObjectLogger.log(output));
            ...
            if (decodedErrorResponse instanceof final MojaveErrorResponse errorResponse) {
                ...
                if (throwable instanceof final PositionReservedExceedsNdcException positionReservedExceedsNdcException) {
                    throw positionReservedExceedsNdcException;
                }
