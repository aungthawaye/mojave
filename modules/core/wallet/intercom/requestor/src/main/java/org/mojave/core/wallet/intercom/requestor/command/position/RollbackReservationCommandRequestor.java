...
@Component
public class RollbackReservationCommandRequestor implements RollbackReservationCommand {

    private static final Logger LOGGER = LoggerFactory.getLogger(RollbackReservationCommandRequestor.class);

    private final Connection connection;

    private final ObjectMapper objectMapper;

    public RollbackReservationCommandRequestor(final Connection connection,
    ...
    public Output execute(final Input input) throws FailedToRollbackReservationException {
        ...
        LOGGER.info("RollbackReservationCommandRequestor : input: ({})", ObjectLogger.log(input));

        try {

            final var output = NatsRequestor.request(
                this.connection, RollbackReservationCommand.SUBJECT_NAME,
                input, Output.class, this.objectMapper);

            LOGGER.info("RollbackReservationCommandRequestor : output : ({})", ObjectLogger.log(output));
            ...
            if (decodedErrorResponse instanceof final MojaveErrorResponse errorResponse) {
                ...
                if (throwable instanceof final FailedToRollbackReservationException failedToRollbackReservationException) {
                    throw failedToRollbackReservationException;
                }
