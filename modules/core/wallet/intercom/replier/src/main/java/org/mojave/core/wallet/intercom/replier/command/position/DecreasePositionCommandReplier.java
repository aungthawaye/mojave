...
@Component
public class DecreasePositionCommandReplier {

    private static final Logger LOGGER = LoggerFactory.getLogger(DecreasePositionCommandReplier.class);

    private final DecreasePositionCommand command;

    private final Connection connection;

    private final ObjectMapper objectMapper;

    public DecreasePositionCommandReplier(final DecreasePositionCommand command,
        ...
        this.connection.createDispatcher(this::handle).subscribe(DecreasePositionCommand.SUBJECT_NAME);
    }
    ...
    private void handle(final Message message) {
        ...
        if (replyTo == null || replyTo.isBlank()) {
            LOGGER.warn("DecreasePositionCommandReplier : reply subject is missing.");
            return;
        }
        ...
        try {
            ...
            final var input = this.objectMapper.readValue(
                message.getData(), DecreasePositionCommand.Input.class);

            LOGGER.info("DecreasePositionCommandReplier : input: ({})", ObjectLogger.log(input));
            ...
            LOGGER.info("DecreasePositionCommandReplier : output : ({})", ObjectLogger.log(output));
