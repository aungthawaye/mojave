...
@Component
public class WithdrawBalanceCommandReplier {

    private static final Logger LOGGER = LoggerFactory.getLogger(WithdrawBalanceCommandReplier.class);

    private final WithdrawBalanceCommand command;

    private final Connection connection;

    private final ObjectMapper objectMapper;

    public WithdrawBalanceCommandReplier(final WithdrawBalanceCommand command,
        ...
        this.connection.createDispatcher(this::handle).subscribe(WithdrawBalanceCommand.SUBJECT_NAME);
    }
    ...
    private void handle(final Message message) {
        ...
        if (replyTo == null || replyTo.isBlank()) {
            LOGGER.warn("WithdrawBalanceCommandReplier : reply subject is missing.");
            return;
        }
        ...
        try {
            ...
            final var input = this.objectMapper.readValue(
                message.getData(), WithdrawBalanceCommand.Input.class);
            ...
            LOGGER.info("WithdrawBalanceCommandReplier : output : ({})", ObjectLogger.log(output));
