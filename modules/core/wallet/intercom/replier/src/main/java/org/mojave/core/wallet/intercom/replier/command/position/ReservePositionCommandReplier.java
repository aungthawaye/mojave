...
@Component
public class ReservePositionCommandReplier {

    private static final Logger LOGGER = LoggerFactory.getLogger(ReservePositionCommandReplier.class);

    private final ReservePositionCommand command;

    private final Connection connection;

    private final ObjectMapper objectMapper;

    public ReservePositionCommandReplier(final ReservePositionCommand command,
        ...
        this.connection.createDispatcher(this::handle).subscribe(ReservePositionCommand.SUBJECT_NAME);
    }
    ...
    private void handle(final Message message) {
        ...
        if (replyTo == null || replyTo.isBlank()) {
            LOGGER.warn("ReservePositionCommandReplier : reply subject is missing.");
            return;
        }
        ...
        try {
            ...
            final var input = this.objectMapper.readValue(
                message.getData(), ReservePositionCommand.Input.class);

            LOGGER.info("ReservePositionCommandReplier : input: ({})", ObjectLogger.log(input));
            ...
            LOGGER.info("ReservePositionCommandReplier : output : ({})", ObjectLogger.log(output));
