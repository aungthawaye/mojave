...
@Service
public class IncreaseNdcCommandHandler implements IncreaseNdcCommand {

    private static final Logger LOGGER = LoggerFactory.getLogger(IncreaseNdcCommandHandler.class);

    private final WalletEngine walletEngine;

    private final WalletCache walletCache;

    public IncreaseNdcCommandHandler(final WalletEngine walletEngine,
    ...
    public Output execute(final Input input) throws
        ...
        LOGGER.info("IncreaseNdcCommand : input: ({})", ObjectLogger.log(input));
        ...
        try {
            ...
            final var history = this.walletEngine.increaseNdc(
                ndcUpdateId, transactionId, transactionAt, walletId, input.amount(),
                input.description());

            final var output = new Output(history.ndcUpdateId());

            LOGGER.info("IncreaseNdcCommand : output: ({})", ObjectLogger.log(output));
        ...
        } catch (final WalletEngine.BalanceLowerThanNewNdcException e) {
            throw new BalanceLowerThanNewNdcException(
                walletId, e.getAmount(), e.getBalance(), e.getNewNdc(), e.getTransactionId());
        }
    }
