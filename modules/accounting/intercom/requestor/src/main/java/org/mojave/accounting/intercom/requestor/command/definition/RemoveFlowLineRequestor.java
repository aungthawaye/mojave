...
@Component
public class RemoveFlowLineRequestor implements RemoveFlowLineCommand {

    private static final Logger LOGGER = LoggerFactory.getLogger(RemoveFlowLineRequestor.class);

    private final Connection connection;

    private final ObjectMapper objectMapper;

    public RemoveFlowLineRequestor(final Connection connection,
    ...
    public Output execute(final Input input) {
        ...
        LOGGER.info("RemoveFlowLineRequestor : input: ({})", ObjectLogger.log(input));

        try {

            final var output = CommandRequestor.request(
                this.connection, RemoveFlowLineCommand.SUBJECT_NAME,
                input, Output.class, this.objectMapper);

            LOGGER.info("RemoveFlowLineRequestor : output : ({})", ObjectLogger.log(output));
