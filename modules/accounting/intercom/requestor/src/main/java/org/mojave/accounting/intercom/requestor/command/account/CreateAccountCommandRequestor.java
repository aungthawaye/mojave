...
@Component
public class CreateAccountCommandRequestor implements CreateAccountCommand {

    private static final Logger LOGGER = LoggerFactory.getLogger(CreateAccountCommandRequestor.class);

    private final Connection connection;

    private final ObjectMapper objectMapper;

    public CreateAccountCommandRequestor(final Connection connection,
    ...
    public Output execute(final Input input) {
        ...
        LOGGER.info("CreateAccountCommandRequestor : input: ({})", ObjectLogger.log(input));

        try {

            final var output = CommandRequestor.request(
                this.connection, CreateAccountCommand.SUBJECT_NAME,
                input, Output.class, this.objectMapper);

            LOGGER.info("CreateAccountCommandRequestor : output : ({})", ObjectLogger.log(output));
