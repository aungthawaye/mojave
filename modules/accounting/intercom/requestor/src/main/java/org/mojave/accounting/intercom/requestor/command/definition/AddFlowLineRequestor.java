...
@Component
public class AddFlowLineRequestor implements AddFlowLineCommand {

    private static final Logger LOGGER = LoggerFactory.getLogger(AddFlowLineRequestor.class);

    private final Connection connection;

    private final ObjectMapper objectMapper;

    public AddFlowLineRequestor(final Connection connection,
    ...
    public Output execute(final Input input) {
        ...
        LOGGER.info("AddFlowLineRequestor : input: ({})", ObjectLogger.log(input));

        try {

            final var output = CommandRequestor.request(
                this.connection, AddFlowLineCommand.SUBJECT_NAME,
                input, Output.class, this.objectMapper);

            LOGGER.info("AddFlowLineRequestor : output : ({})", ObjectLogger.log(output));
