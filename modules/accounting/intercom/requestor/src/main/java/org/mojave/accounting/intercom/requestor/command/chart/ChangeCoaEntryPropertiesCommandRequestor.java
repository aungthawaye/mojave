...
@Component
public class ChangeCoaEntryPropertiesCommandRequestor implements ChangeCoaEntryPropertiesCommand {

    private static final Logger LOGGER = LoggerFactory.getLogger(ChangeCoaEntryPropertiesCommandRequestor.class);

    private final Connection connection;

    private final ObjectMapper objectMapper;

    public ChangeCoaEntryPropertiesCommandRequestor(final Connection connection,
    ...
    public Output execute(final Input input) {
        ...
        LOGGER.info("ChangeCoaEntryPropertiesCommandRequestor : input: ({})", ObjectLogger.log(input));

        try {

            final var output = CommandRequestor.request(
                this.connection, ChangeCoaEntryPropertiesCommand.SUBJECT_NAME,
                input, Output.class, this.objectMapper);

            LOGGER.info("ChangeCoaEntryPropertiesCommandRequestor : output : ({})", ObjectLogger.log(output));
