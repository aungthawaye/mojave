...
@Component
public class AddFlowLineReplier {

    private static final Logger LOGGER = LoggerFactory.getLogger(AddFlowLineReplier.class);
    ...
    public AddFlowLineReplier(final AddFlowLineCommand addFlowLineCommand,
        ...
        this.connection.createDispatcher(this::handle).subscribe(AddFlowLineCommand.SUBJECT_NAME);
    }
    ...
    private void handle(final Message message) {
        ...
        if (replyTo == null || replyTo.isBlank()) {
            LOGGER.warn("AddFlowLineReplier : reply subject is missing.");
            return;
        }
        ...
        try {
            ...
            final var input = this.objectMapper.readValue(
                message.getData(), AddFlowLineCommand.Input.class);

            LOGGER.info("AddFlowLineReplier : input: ({})", ObjectLogger.log(input));

            final var output = this.addFlowLineCommand.execute(input);
            ...
            LOGGER.info("AddFlowLineReplier : output : ({})", ObjectLogger.log(output));
