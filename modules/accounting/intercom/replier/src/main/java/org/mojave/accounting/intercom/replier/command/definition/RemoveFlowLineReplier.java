...
@Component
public class RemoveFlowLineReplier {

    private static final Logger LOGGER = LoggerFactory.getLogger(RemoveFlowLineReplier.class);
    ...
    public RemoveFlowLineReplier(final RemoveFlowLineCommand removeFlowLineCommand,
        ...
        this.connection.createDispatcher(this::handle).subscribe(RemoveFlowLineCommand.SUBJECT_NAME);
    }
    ...
    private void handle(final Message message) {
        ...
        if (replyTo == null || replyTo.isBlank()) {
            LOGGER.warn("RemoveFlowLineReplier : reply subject is missing.");
            return;
        }
        ...
        try {
            ...
            final var input = this.objectMapper.readValue(
                message.getData(), RemoveFlowLineCommand.Input.class);

            LOGGER.info("RemoveFlowLineReplier : input: ({})", ObjectLogger.log(input));

            final var output = this.removeFlowLineCommand.execute(input);
            ...
            LOGGER.info("RemoveFlowLineReplier : output : ({})", ObjectLogger.log(output));
