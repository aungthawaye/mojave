...
import org.mojave.accounting.contract.command.account.ActivateAccountCommand;
...
@Component
public class ActivateAccountCommandReplier {

    private static final Logger LOGGER = LoggerFactory.getLogger(ActivateAccountCommandReplier.class);
    ...
    public ActivateAccountCommandReplier(final ActivateAccountCommand activateAccountCommand,
        ...
        this.connection.createDispatcher(this::handle).subscribe(ActivateAccountCommand.SUBJECT_NAME);
    }
    ...
    private void handle(final Message message) {
        ...
        if (replyTo == null || replyTo.isBlank()) {
            LOGGER.warn("ActivateAccountCommandReplier : reply subject is missing.");
            return;
        }
        ...
        try {
            ...
            final var input = this.objectMapper.readValue(
                message.getData(), ActivateAccountCommand.Input.class);

            LOGGER.info("ActivateAccountCommandReplier : input: ({})", ObjectLogger.log(input));
            ...
            LOGGER.info("ActivateAccountCommandReplier : output : ({})", ObjectLogger.log(output));
