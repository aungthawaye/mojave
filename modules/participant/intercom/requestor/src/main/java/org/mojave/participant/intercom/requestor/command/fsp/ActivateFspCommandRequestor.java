...
@Component
public class ActivateFspCommandRequestor implements ActivateFspCommand {

    private static final Logger LOGGER = LoggerFactory.getLogger(ActivateFspCommandRequestor.class);

    private final Connection connection;

    private final ObjectMapper objectMapper;

    public ActivateFspCommandRequestor(final Connection connection, final ObjectMapper objectMapper) {
    ...
    public Output execute(final Input input) {
        ...
        LOGGER.info("ActivateFspCommandRequestor : input: ({})", ObjectLogger.log(input));

        try {

            final var output = CommandRequestor.request(
                this.connection, ActivateFspCommand.SUBJECT_NAME,
                input, Output.class, this.objectMapper);

            LOGGER.info("ActivateFspCommandRequestor : output : ({})", ObjectLogger.log(output));
