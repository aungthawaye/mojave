...
@Component
public class DeactivateFspCommandRequestor implements DeactivateFspCommand {

    private static final Logger LOGGER = LoggerFactory.getLogger(DeactivateFspCommandRequestor.class);

    private final Connection connection;

    private final ObjectMapper objectMapper;

    public DeactivateFspCommandRequestor(final Connection connection, final ObjectMapper objectMapper) {
    ...
    public Output execute(final Input input) {
        ...
        LOGGER.info("DeactivateFspCommandRequestor : input: ({})", ObjectLogger.log(input));

        try {

            final var output = CommandRequestor.request(
                this.connection, DeactivateFspCommand.SUBJECT_NAME,
                input, Output.class, this.objectMapper);

            LOGGER.info("DeactivateFspCommandRequestor : output : ({})", ObjectLogger.log(output));
