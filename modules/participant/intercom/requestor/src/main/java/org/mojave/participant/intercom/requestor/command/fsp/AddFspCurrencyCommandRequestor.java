...
@Component
public class AddFspCurrencyCommandRequestor implements AddFspCurrencyCommand {

    private static final Logger LOGGER = LoggerFactory.getLogger(AddFspCurrencyCommandRequestor.class);

    private final Connection connection;

    private final ObjectMapper objectMapper;

    public AddFspCurrencyCommandRequestor(final Connection connection, final ObjectMapper objectMapper) {
    ...
    public Output execute(final Input input) {
        ...
        LOGGER.info("AddFspCurrencyCommandRequestor : input: ({})", ObjectLogger.log(input));

        try {

            final var output = CommandRequestor.request(
                this.connection, AddFspCurrencyCommand.SUBJECT_NAME,
                input, Output.class, this.objectMapper);

            LOGGER.info("AddFspCurrencyCommandRequestor : output : ({})", ObjectLogger.log(output));
