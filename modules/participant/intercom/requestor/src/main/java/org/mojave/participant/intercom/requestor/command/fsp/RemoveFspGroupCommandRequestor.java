...
@Component
public class RemoveFspGroupCommandRequestor implements RemoveFspGroupCommand {

    private static final Logger LOGGER = LoggerFactory.getLogger(RemoveFspGroupCommandRequestor.class);

    private final Connection connection;

    private final ObjectMapper objectMapper;

    public RemoveFspGroupCommandRequestor(final Connection connection, final ObjectMapper objectMapper) {
    ...
    public Output execute(final Input input) {
        ...
        LOGGER.info("RemoveFspGroupCommandRequestor : input: ({})", ObjectLogger.log(input));

        try {

            final var output = CommandRequestor.request(
                this.connection, RemoveFspGroupCommand.SUBJECT_NAME,
                input, Output.class, this.objectMapper);

            LOGGER.info("RemoveFspGroupCommandRequestor : output : ({})", ObjectLogger.log(output));
