...
@Component
public class IncreasePositionCommandRequestor implements IncreasePositionCommand {

    private static final Logger LOGGER = LoggerFactory.getLogger(IncreasePositionCommandRequestor.class);

    private final Connection connection;

    private final ObjectMapper objectMapper;

    public IncreasePositionCommandRequestor(final Connection connection,
    ...
    public Output execute(final Input input) {
        ...
        LOGGER.info("IncreasePositionCommandRequestor : input: ({})", ObjectLogger.log(input));

        try {

            final var output = CommandRequestor.request(
                this.connection, IncreasePositionCommand.SUBJECT_NAME,
                input, Output.class, this.objectMapper);

            LOGGER.info("IncreasePositionCommandRequestor : output : ({})", ObjectLogger.log(output));
