...
@Service
public class UpdateQuotesResponseStepHandler implements UpdateQuotesResponseStep {

    private static final Logger LOGGER = LoggerFactory.getLogger(
        UpdateQuotesResponseStepHandler.class);

    private final QuoteRepository quoteRepository;

    public UpdateQuotesResponseStepHandler(QuoteRepository quoteRepository) {
    ...
    public void execute(Input input) throws FspiopException {
        ...
        LOGGER.info("UpdateQuotesResponseStep : input : ({})", ObjectLogger.log(input));
        ...
        try {
            ...
            if (optQuote.isEmpty()) {
                ...
                LOGGER.warn(
                    "UpdateQuotesResponseStep : quote not found. udfQuoteId : ({})",
            ...
            quote.responded(
                input.responseExpiration(), input.transferAmount(), input.payeeFspFee(),
                input.payeeFspCommission(), input.payeeReceiveAmount(), input.ilpPacket(),
                input.condition());
            ...
            LOGGER.info(
                "UpdateQuotesResponseStep : done, took {} ms", (endAt - startAt) / 1_000_000);
