...
Use this skill to reason like a senior software architect for regulated payment platforms. Produce designs that keep messaging semantics, payment state transitions, ledger movements, reconciliation, settlement, and operational controls aligned.
...
- Ledger model: memo only, subledger plus GL, or multi-entity and multi-currency ledger.
...
- Read `references/ledger-and-accounting.md` when the task involves booking logic, holds, postings, suspense handling, settlement positions, reconciliation, break management, or finance controls.
...
- Enumerate actors and boundaries: channel, API gateway, payment switch, sanctions or fraud, scheme adapter, settlement engine, ledger, reconciliation, operations, and reporting.
...
- Define reconciliations among switch events, scheme statements, settlement positions, and ledger balances.
- Define observability around value at risk, stuck states, retry storms, and message backlog.

### 6. Stress The Failure Paths

- Walk through timeout after scheme submission, late confirmation, double delivery, partial posting, downstream ledger outage, settlement mismatch, and replay after recovery.
...
- Do not treat a switch as a general ledger.
...
- Do not present ledger entries without naming the triggering business event and finality condition.
...
- Explain how to reconcile scheme settlement files against switch activity and ledger balances.
